...
try:
    ...
    from on_os import get_weather_report, search_on_google, search_on_wikipedia, send_email, play_on_youtube, play_song, \
        get_random_joke, find_my_ip, get_latest_news, get_random_advice, get_trending_movies, read_news_about, \
        open_website
...
def changeTheme():
    ...
    settingsLbl['fg'], userPhoto['fg'], userName['fg'], themeLbl['fg'], chooseChatLbl[
        'fg'] = textColor, textColor, textColor, textColor, textColor
    settingsLbl['bg'], userPhoto['bg'], userName['bg'], themeLbl['bg'], chooseChatLbl[
...
def main(text):
    ...
    if isContain(text, ['thời tiết']):
        ...
        speak(
            f"Nhiệt độ hiện tại là {temperature}, độ ẩm hiện tại là  {feels_like}, áp suất không khí hiện tại là  {pressure}",
            True)
        return
...
if __name__ == '__main__':
    ...
    darkBtn = ttk.Radiobutton(settingsFrame, text='Tối', value=1, variable=themeValue, style='Wild.TRadiobutton',
                              command=changeTheme, takefocus=False)
    darkBtn.place(x=150, y=145)
    lightBtn = ttk.Radiobutton(settingsFrame, text='Sáng', value=2, variable=themeValue, style='Wild.TRadiobutton',
                               command=changeTheme, takefocus=False)
    ...
    backBtn = Button(settingsFrame, text='   Trở về   ', bd=0, font=('Arial 12'), fg='white', bg='#14A769', relief=FLAT,
                     command=lambda: raise_frame(root1))
    clearFaceBtn = Button(settingsFrame, text='   Đóng   ', bd=0, font=('Arial 12'), fg='white', bg='#14A769',
                          relief=FLAT, command=deleteUserData)
